    ...
    def post():
        ...
        if sessionOperationClass.check():
            ...
            if role == 1:
                ...
                return resp
            abort(HTTPStatus.BadRequest, **make_result(status=APIStatus.BadRequest, msg='仅限后台用户发布'))
        abort(HTTPStatus.BadRequest, **make_result(status=APIStatus.BadRequest, msg='未登录用户'))
    ...
    def put(id):
        ...
        if sessionOperationClass.check():
            ...
            if role == 1:
                ...
                return resp
            abort(HTTPStatus.BadRequest, **make_result(status=APIStatus.BadRequest, msg='仅限后台用户修改'))
        abort(HTTPStatus.BadRequest, **make_result(status=APIStatus.BadRequest, msg='未登录用户'))
    ...
    def delete(id):
        ...
        if sessionOperationClass.check():
            ...
            if role == 1:
                ...
                return resp
            abort(HTTPStatus.BadRequest, **make_result(status=APIStatus.BadRequest, msg='仅限后台用户删除'))
        abort(HTTPStatus.BadRequest, **make_result(status=APIStatus.BadRequest, msg='未登录用户'))
