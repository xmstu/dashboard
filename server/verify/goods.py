...
def goods_list_check_params(params):
    try:
        ...
        # 补全时间
        params['create_start_time'], params['create_end_time'] = complement_time(params['create_start_time'],
...
def cancel_goods_reason_check_params(params):
    try:
        if not SessionOperationClass.check():
            abort(HTTPStatus.BadRequest, **make_resp(status=APIStatus.UnLogin, msg='请登录'))
...
def goods_distribution_trend_check_params(params):
    try:
        if not SessionOperationClass.check():
            abort(HTTPStatus.BadRequest, **make_resp(status=APIStatus.UnLogin, msg='请登录'))
