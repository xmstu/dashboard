...
def get_nearby_driver(goods, filter_type=1):
    ...
    for i in all_drivers:
        ...
        if filter_type == 1:
            ...
            if result and result['province'] == goods['from_province_id'] and result['city'] == goods['from_city_id'] \
                    and result.get('location_time') \
                    and int(time.time() - time.mktime(time.strptime(result['location_time'], '%Y-%m-%d %H:%M:%S'))) < 172800:
                # 车长
                length_id = str(i['vehicle_length_id']).split(',')[0]
                user_info = VehicleModel.get_vehicle_length_name(db.read_db, int(length_id), i['user_id'])
                # 车长大于货源需求
        ...
        if len(driver) >= 10:
            break
    if not driver:
        driver = get_nearby_driver(goods, filter_type=2)
    return driver
