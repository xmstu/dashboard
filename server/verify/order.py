...
def orders_received_statistics_check_params(params):
    try:
        ...
        log.error('订单趋势请求参数有误:[Error:{}]'.format(e), exc_info=True)
        abort(HTTPStatus.BadRequest, **make_resp(status=APIStatus.Forbidden, msg='订单趋势请求参数有误'))


def cancel_order_check_params(params):
    try:
        if not SessionOperationClass.check():
            abort(HTTPStatus.BadRequest, **make_resp(status=APIStatus.UnLogin, msg='请登录'))
        params['start_time'] = int(params.get('start_time', None) or time.time() - 86400 * 7)
...
def order_list_check_params(params):
    try:
        if not SessionOperationClass.check():
            abort(HTTPStatus.BadRequest, **make_resp(status=APIStatus.UnLogin, msg='请登录'))
