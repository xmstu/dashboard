...
def get_struct_data(data, params, *args, **kwargs):
    ...
    for count in data:
        if count.get('create_time'):
            ...
            date_count[create_time] = [count.get(i, 0) for i in args] * 2 if len(args) > 1 else count.get(args[0], 0)
    # 日期补全
    ...
    xAxis = []
    series = []
    if not params.get('periods'):
        xAxis = list(date_count.keys())
        series = list(date_count.values())

    elif params.get('periods') == 2:
        date_val = begin_date
...
data_price = {
    ...
    '小面包车': CarPrice(2.54, -4.3/1000, 0.00000644, 26.38, 30),
    # =28.79+2.8*A3+(-3.85)*(10^-3)*A3^2+4.56*(10^-6)*A3^3
    '中面包车': CarPrice(2.8, -3.85/1000, 0.00000456, 28.79, 30),
    # =28.08+3.18*A3+(-4.84)*(10^-3)*A3^2+5.37*(10^-6)*A3^3
    '小货车': CarPrice(3.18, -4.84/1000, 0.00000537, 28.08, 30),
    # =55.17+3.96*A3+(-3.9)*(10^-3)*A3^2+5.21*(10^-6)*A3^3
    '4.2米': CarPrice(3.96, -3.9/1000, 0.00000521, 55.17, 30),
    # =1.04*(10^2)+4.52*A3-2.28*(10^-3)*A3^2
    '5.2米': CarPrice(4.52, -2.28/1000, 0, 104, 30),
    # =2.25*(10^2)+3.7*A3+2.52*(10^-3)*A3^2-1.85*(10^-6)*A3^3
    '6.8米': CarPrice(3.7, 2.52/1000, -0.00000185, 225, 30),
    # =2.07*(10^2)+4.77*A4+(-1.45)*(10^-3)*A4^2+2.01*(10^-6)*A4^3
    '7.6米': CarPrice(4.77, -1.45/1000, 0.00000201, 207, 30),
    # =3.29*(10^2)+6.88*A3+(-1.91)*(10^-3)*A3^2+5.84*(10^-7)*A3^3
    '9.6米': CarPrice(6.88, -1.91/1000, 0.000000584, 329, 30),
    # =6.56*(10^2)+9.88*A2-6.26*(10^-3)*A2^2
    '13米': CarPrice(9.88, -6.26/1000, 0, 656, 30),
    # =1.02*(10^3)+11.2*A2-7.48*(10^-3)*A2^2
    '17.5米': CarPrice(11.2, -7.48/1000, 0, 1020, 30),
}
